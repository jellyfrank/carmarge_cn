...
class sale_cost(models.Model):
    ...
    product_id = fields.Many2one("product.product",string="产品", domain="[('type','=','service'),('is_cost_service','=',True)]")
    ...
    def button_split(self):
        ...
        if  self.split_method == "equal":
            ...
            for line in self.order_id.order_line:
                line.price_unit += additional_cost / line.product_uom_qty if line.product_uom_qty else 0
            ...
            for line in self.order_id.order_line:
                line.price_unit  += additional_cost * line.product_id.standard_price / line.product_uom_qty if line.product_uom_qty else 0
            ...
            for line in self.order_id.order_line:
                line.price_unit += additional_cost * line.product_id.weight / line.product_uom_qty if line.product_uom_qty else 0
        else:
            vol = sum(self.order_id.order_line.mapped("product_id").mapped("volume"))
            additional_cost = self.cost / vol if vol else 0
            for line in self.order_id.order_line:
                line.price_unit += additional_cost * line.product_id.volume / line.product_uom_qty if line.product_uom_qty else 0
